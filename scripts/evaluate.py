...
def main():
    ...
    if not rows:
        print("No findings fund, skipping evaluation.")
        sys.exit(0)
    ...
    for message, file in rows:
        data['question'].append("what issue exist in this code?")
        data['answer'].append(message or "")
        data["context"].append(file or "")

    dataset = Dataset.from_dict(data)
    results = evaluate(dataset, metrix=[faithfulness, answer_relevancy])
