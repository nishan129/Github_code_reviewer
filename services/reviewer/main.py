...
from model import Settings, PullRequest,ReviewRequest
...
def _finding_summary_line(f:dict) -> str:
    ...
    return f"**[{severity}]** `{f.get("file","unknow")}:{f.get('line',"?")}` ({f.get('agent','')})\n{f.get('message',"")}"
...
async def post_review(request: ReviewRequest):
    ...
    async with AsyncSessionLocal() as session:
        await session.execute(
            update(PullRequest).where(PullRequest.id == request.pr_id).values(status='reviewd')
        )
...
def get_installation_token(installation_id: int) -> str:
    now = int(time.time())
    payload = {'int':now - 60, "exp": now + 600, "iss": settings.github_app_id}
