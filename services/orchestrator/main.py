...
async def analyze(request: AnalyzeRequest):
    ...
    async with AsyncSessionLocal() as session:
        ...
        findngs_data = state.get("findings",[])


        async with AsyncSessionLocal() as session:
            for f in findngs_data:
                session.add(Finding(
                    pr_id = request.pr_id,
                    file = f.get('file'),
                    line = f.get("line"),
        ...
        async with httpx.AsyncClient() as client:
            await client.post(
                "http://reviewer:8--3/post-review",
                json = {
                    "pr_id": str(request.pr_id),
                    "repo_full_name": request.repo_full_name,
                    "pr_number": request.pr_number,
                    "installation_id": request.installation_id,
                    "findings": findngs_data
                },
                timeout=60,
            )
...
async def fetch_diff(repo_full_name:str, pr_number:int, token:str) -> str:
    async with httpx.AsyncClient() as client:
        response = await client.get(
            f"https://api.github.con/repos/{repo_full_name}/pulls/{pr_number}",
            headers = {
